...
{#snippet avatar(name: string, size: string)}
  <span class="{size} rounded-full grid place-items-center font-extrabold text-accent-ink shrink-0"
        style="background: radial-gradient(circle at 50% 34%, #cf5a68, var(--color-accent) 60%, #5f1420)">{initial(name)}</span>
{/snippet}
...
<div class="wrap">
  ...
  {:else}
    ...
    <div class="grid grid-cols-2 gap-1 bg-surface-2 border border-border rounded-xl p-1 mb-4">
      <button class="py-2 rounded-lg font-semibold text-sm transition-all {tab === 'feed' ? 'bg-gradient-to-b from-accent to-[#66131f] text-accent-ink shadow-md' : 'text-muted hover:text-text'}"
        onclick={() => (tab = 'feed')}>Feed</button>
      <button class="py-2 rounded-lg font-semibold text-sm transition-all {tab === 'leaderboard' ? 'bg-gradient-to-b from-accent to-[#66131f] text-accent-ink shadow-md' : 'text-muted hover:text-text'}"
